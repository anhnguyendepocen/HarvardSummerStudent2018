...
# Simple barplot
topWords<-subset(beerFreq, beerFreq$frequency>=90) #values greater than 90
topWords <- topWords[order(topWords$frequency, decreasing=F),] #reorder from top to bottom
topWords$word<-factor(topWords$word, levels=unique(as.character(topWords$word))) # change to factor for ggplot
...
# Reduce TDM
beerTDM2 <- removeSparseTerms(beerTDM, sparse=0.97) #shoot for ~50 terms; higher sparse=higher 3 of terms 
beerTDM2

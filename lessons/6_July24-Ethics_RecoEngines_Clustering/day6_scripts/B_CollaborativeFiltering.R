...
songData[songData$song_id %in% unlist(recoList[[3]]),]

# What did this listener originally listen too?  Does the reco make sense?
singleUser <- names(recoList)[3] #"8ee90038724c4957eb4df16f3e9c6ed2b570a3ec"

# Subset to a single user then join the data
singleListens <- subset(songListens, songListens$userID == singleUser)
